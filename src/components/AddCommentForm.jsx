const AddCommentForm = ({
  ...
  <div className="bg-white/20 rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
    <h3 className="text-base sm:text-lg font-semibold text-white/80 mb-3">
      Add a Comment
    </h3>
    <textarea
      value={newComment}
      onChange={(e) => setNewComment(e.target.value)}
      placeholder="What are your thoughts?"
      className="w-full bg-white/40 p-2 sm:p-3 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none placeholder:text-white/70 text-sm sm:text-base"
      rows="4"
    />
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-3 gap-3">
      ...
      <button
        onClick={onSubmit}
        disabled={!newComment.trim()}
        className="w-full sm:w-auto px-4 sm:px-6 py-2 bg-teal-600 font-semibold text-white rounded-lg hover:bg-teal-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors text-sm sm:text-base"
      >
        Post Comment
      </button>
    </div>
